...
public class BaseController {
    ...
    // 设置cookie-val
    public void setCookieVal(HttpServletResponse resp, HttpServletRequest req, String name, String val) {
        if (val == null) {
            val = "";
        }
...
//			Java版本低于JDK 11，那么cookie.setSameSite方法是不可用的
//			setSameSite  普通的Cookie属性。其作用是限制该Cookie的在请求的时候是否被传递到Cookie所属服务的场景。
...
//			Java版本低于JDK 11，setSameSite解决方法
            String encodedVal = URLEncoder.encode(val, "UTF-8");
            String cookieValue = name + "=" + encodedVal + ";path=/;HttpOnly;Domain=localhost;SameSite=None; Secure";
            resp.setHeader("Set-Cookie", cookieValue);
            resp.setHeader("Access-Control-Allow-Credentials", "true");
            resp.setHeader("Access-Control-Allow-Origin", req.getHeader("Origin"));//谁访问我，我允许谁
    ...
    public <T> ArrayList<T> getCookieList(HttpServletRequest req, HttpServletResponse resp, Class<T> clazz,
        ...
        if (null == cklistStr || "".equals(cklistStr.trim())) {
            ...
            this.setCookieVal(resp, req, listName, JSON.toJSONString(list));
        }
        // 返回list
